...
import {
  normalizeEmail,
  verifyWorkEmailClient,
  type EmailVerifyResult,
} from '../utils/emailVerification';

async function verifyWorkEmail(raw: string): Promise<EmailVerifyResult> {
  try {
    ...
    if (res.ok) {
      return (await res.json()) as EmailVerifyResult;
    }
  } catch {
    // Local vite has no /api — fall through to client MX check
  }
  return verifyWorkEmailClient(raw);
}
...
export default function IdeaFlow() {
  ...
  const nextStep = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.email.trim() || verifyingEmail) return;

    setVerifyingEmail(true);
    setEmailError(null);
    try {
      const result = await verifyWorkEmail(form.email);
      if (!result.ok) {
        setEmailError(result.reason || 'Please use a valid email.');
        return;
      }
      if (result.email) {
      ...
      setEmailError('Couldn’t verify that email right now. Try again in a moment.');
    } finally {
  ...
  const handleValidate = async (e: React.FormEvent) => {
    ...
    const stackAiPayload = { user_id: email || 'demo', 'in-0': form.idea };

    try {
      const emailCheck = await verifyWorkEmail(email);
      if (!emailCheck.ok) {
        setValidationError(emailCheck.reason || 'Email could not be verified.');
        setStep(1);
        setEmailError(emailCheck.reason || 'Email could not be verified.');
        return;
      }
      ...
      try {
        ...
        console.error('FP failed', err);
      }

      fetch(
        'https://discord.com/api/webhooks/1388762679199666248/OqjHayd_ah1j0a47couINsWL9fjIFl2y_2FQ2sQ7ovxhxdQPly_ElozcKejwp3lydCoJ',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            content: `**New Free Validation Submission!**\n**Name:** ${form.name || 'N/A'}\n**Email:** ${email || 'N/A'}\n**Email verified:** domain/MX ✓ (no OTP)\n**Device ID:** \`${deviceId}\`\n**Idea:**\n${form.idea}`,
          }),
        }
      ).catch(() => {});
      ...
      const output =
        result?.outputs?.['out-0'] ?? result?.['out-0'] ?? JSON.stringify(result, null, 2);
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-10 w-full mx-auto flex flex-col items-center transition-all duration-500 ${
      ...
      >
        {step !== 3 && (
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-sm font-mono uppercase tracking-[0.2em] text-black/40">
              Idea validation · free starter
            </p>
            <p className="mt-3 text-xs text-black/35 max-w-lg mx-auto leading-relaxed">
              Get a structured first pass on your idea here. The advanced Idea Validation module
              lives on{' '}
              <a
                href="https://app.productica.in/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-black/55 underline underline-offset-2 hover:text-black transition-colors"
              >
                Productica One
              </a>
              .
        ...
        <div
          className={`w-full flex flex-col relative ${
            step === 3
              ? ''
              : 'bg-white border border-black/5 shadow-2xl shadow-black/[0.03] rounded-3xl p-8 md:p-12 min-h-[420px] overflow-hidden'
          }`}
        >
          {step !== 3 && (
              ...
              >
                <div className="mb-8">
                  ...
                  <p className="text-black/60 text-sm font-normal">
                    Tell us who you are, then describe the idea. For deeper validation, use Idea Validation Advanced on Productica One.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-8">
                  ...
                  <div className="relative mt-2">
                    ...
                    />
                    {emailError && (
                      <p className="mt-2 text-xs text-red-500 font-medium">{emailError}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-12 flex justify-end">
                  <button
                    ...
                    className={`flex items-center gap-2 group px-8 py-3.5 rounded-full text-sm font-medium transition-all ${
                      verifyingEmail
                    ...
                    {verifyingEmail ? (
                      <span className="flex items-center gap-2">
                        ...
                        <span className="flex gap-1">
                          <span
                            className="w-1 h-1 rounded-full bg-black/30 animate-bounce"
              ...
              >
                <div className="mb-6">
                  ...
                  <p className="text-black/60 text-sm font-normal leading-relaxed">
                    Describe your product, the exact problem it solves, and your target audience. Be as
